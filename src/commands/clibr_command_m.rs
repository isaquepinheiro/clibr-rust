...
impl ICommand for CommandModule {
    fn execute(&self, _dir_name: &str, _file_name: &str, _cli: &mut dyn ICli) -> bool {
        ...
        let template_file_path: String = IfThen::<String>::new()
            .when(is_app_module)
        ...
        let mut modified_content: String = template_content
            .to_string()
...
impl CommandModule {
    ...
    pub fn generate_guard_body(&self, _camel_casename: &str, _cli: &mut dyn ICli) -> String {
        ...
        let modified_content: String =
            template_content.replace("{className}", &format!("T{}", _camel_casename));
