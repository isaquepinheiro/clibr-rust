...
#[derive(Default)]
pub struct CommandRouteHandlerHorse {}

impl CommandRouteHandlerHorse {
    pub fn new() -> Self {
        Self {}
    }
}

impl ICommand for CommandRouteHandlerHorse {
    fn execute(&self, _dir_name: &str, _file_name: &str, _cli: &mut dyn ICli) -> bool {
        ...
        let class_name: String = format!("T{}RouteHandler", &camel_casename);
        ...
        let template_file_path: String = format!("{}/horse.handler.pas", _cli.get_path_temp());
        let template_file_name: String = format!("{}/{}.route.handler.pas", &source_path, &unit_name);
        ...
        let modified_content: String = template_content.to_string()
            ...
            .replace("{handlerName}", &class_name)
        ...
        let update: String = format!(
            "  {}.route.handler in 'src\\modules\\{}\\{}.route.handler.pas',",
